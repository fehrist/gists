...
const $9ea3a24d21594cd9$export$490f559806196b34 = {
  // DELETE
  getProductsListContainer: ({ id: id })=>`<div id="${id}" class="fehris-products-list"></div>`,
  // DELETE
  getProductCard: ({ name: name, img: img, id: id, formatted_price: formatted_price, html_url: html_url })=>{
      return `
  ...
  getProductsListContainer2: ({ id: id })=>`<div id="${id}" class="cardsContainer py-xl"></div>`,
  getProductCard2: ({ name: name, img: img, id: id, formatted_price: formatted_price, html_url: html_url, categoryName: categoryName })=>`
 ...
 */ const appendFilterToElement = ({ language: language, containerId: containerId, filter: filter })=>{
  ...
  const appendProductsToElement = ({ products: products, containerId: containerId, language: language })=>{
      ...
      const container = document.getElementById(containerId);
      (0, $f66fdc2e8eaf2d73$export$d8a0fc79d6aedf2e)(container, (0, $9ea3a24d21594cd9$export$490f559806196b34).getProductsListContainer2({
          id: PRODUCTS_LIST_ID
      }));
      const plContainer = document.getElementById(PRODUCTS_LIST_ID);
      products.forEach((p)=>{
          const { name: name, id: id, html_url: html_url, formatted_price: formatted_price, images: images, categories: categories } = p ?? {};
          const productHtml = (0, $9ea3a24d21594cd9$export$490f559806196b34).getProductCard2({
              name: name[language],
              id: id,
              html_url: html_url,
